...
public class ImageCache
{
    ...
    private void init( Context context, ImageCacheParams cacheParams )
    {
        ...
        if ( cacheParams.diskCacheEnabled )
        {
            ...
            if ( getUsableSpace( diskCacheDir ) > cacheParams.diskCacheSize )
            {
                ...
                {
                    mICSDiskCache =
                        ICSDiskLruCache.open( diskCacheDir, 1, 1, cacheParams.diskCacheSize );
                }
        ...
        if ( mICSDiskCache != null )
        {
            try
            {
                ...
                if ( snapshot != null )
                {
                    ...
                    return BitmapFactory.decodeStream( snapshot.getInputStream( ICS_DISK_CACHE_INDEX ) );
                }
            }
            catch ( final IOException e )
            {
     ...
     * changes. In this sample it will be used to retain the ImageCache object.
     */
