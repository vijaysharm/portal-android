...
import static com.intelerad.android.portal.LogUtils.LOGD;
import static com.intelerad.android.portal.LogUtils.LOGE;
import static com.intelerad.android.portal.LogUtils.LOGV;
import static com.intelerad.android.portal.LogUtils.makeLogTag;
...
public class ImageFetcher extends ImageWorker
{
    ...
    public void loadThumbnailImage( Session session, String key, ImageView imageView, Bitmap loadingBitmap )
    {
        loadImage( new ImageData( session, key, ImageData.IMAGE_TYPE_THUMBNAIL ), imageView, loadingBitmap );
    }

    public void loadThumbnailImage( Session session, String key, ImageView imageView, int resId )
    {
        loadImage( new ImageData( session, key, ImageData.IMAGE_TYPE_THUMBNAIL ), imageView, resId );
    }

    public void loadThumbnailImage( Session session, String key, ImageView imageView )
    {
        loadImage( new ImageData( session, key, ImageData.IMAGE_TYPE_THUMBNAIL ), imageView, mLoadingBitmap );
    }

    public void loadImage( Session session, String key, ImageView imageView, Bitmap loadingBitmap )
    {
        loadImage( new ImageData( session, key, ImageData.IMAGE_TYPE_NORMAL ), imageView, loadingBitmap );
    }

    public void loadImage( Session session, String key, ImageView imageView, int resId )
    {
        loadImage( new ImageData( session, key, ImageData.IMAGE_TYPE_NORMAL ), imageView, resId );
    }

    public void loadImage( Session session, String key, ImageView imageView )
    {
        loadImage( new ImageData( session, key, ImageData.IMAGE_TYPE_NORMAL ), imageView, mLoadingBitmap );
    }
     ...
     */
    private Bitmap processBitmap( Session session, String key, int type )
    {
        ...
        if ( type == ImageData.IMAGE_TYPE_NORMAL )
        {
            ...
            if ( f != null )
            {
                ...
                final Bitmap bitmap =
                    decodeSampledBitmapFromFile( f.toString(),
                                                 mFetcherParams.mImageWidth,
        ...
        else if ( type == ImageData.IMAGE_TYPE_THUMBNAIL )
        {
            ...
            final byte[] bitmapBytes =
                downloadBitmapToMemory( session, mContext, key, mFetcherParams.mMaxThumbnailBytes );
    ...
    protected Bitmap processBitmap( Object key )
    {
        ...
        return processBitmap( imageData.mSession, imageData.mKey, imageData.mType );
    }
     ...
     */
    public static byte[] downloadBitmapToMemory( Session session, Context context, String urlString, int maxBytes )
    {
        try
        {
            return PortalApi.getImageBytes( session, urlString );
        }
    ...
    private static class ImageData
    {
        ...
        public ImageData( Session session, String key, int type )
        {
            mSession = session;
            mKey = key;
            mType = type;
